...
async function loadYearEntries(
  ...
  type SliderAcc = { sweetness: number; body: number; heat: number; char: number; linger: number; balance: number; count: number };
  ...
  for (const t of (groupRaw ?? []) as GroupTastingRow[]) {
    ...
    const acc = sliderAcc.get(t.whiskey_day_id) ?? {
      sweetness: 0, body: 0, heat: 0, char: 0, linger: 0, balance: 0, count: 0,
    };
    acc.sweetness += s.sweetness ?? 0;
    acc.body      += s.body      ?? 0;
    acc.heat      += s.heat      ?? 0;
    acc.char      += s.char      ?? 0;
    acc.linger    += s.linger    ?? 0;
    ...
    .map((t): SearchEntry | null => {
      ...
      const groupSliders: GroupSliders = acc
        ? {
            ...
            heat:      avg(acc, "heat"),
            char:      avg(acc, "char"),
            linger:    avg(acc, "linger"),
            balance:   avg(acc, "balance"),
          }
        : { sweetness: null, body: null, heat: null, char: null, linger: null, balance: null };

      return {
        whiskey_day_id: t.whiskey_day_id,
        season_year: year,
        day_number: (day.day_number ?? 0) as number,
